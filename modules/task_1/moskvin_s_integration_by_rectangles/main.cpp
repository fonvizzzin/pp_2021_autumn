...
TEST(parallel_operations_mpi, can_integration_correct_using_sequential_method) {
    ...
    if (rank == 0) {
        ...
        double seq_res = sequentialIntegration(0, 2, 10, Func);
        double res = 2;
...
TEST(parallel_operations_mpi, can_integration_correct_using_parallel_method) {
    ...
    double par_res = parallelIntegration(0, 2, 10, Func);
...
TEST(test_different_submethods, integration_sequential_submethods) {
    ...
    if (rank == 0) {
        ...
        double seq_res = sequentialIntegration(0, 2, 1000, Func);
        double res = 2;
        ASSERT_NEAR(res, seq_res, error);
    }
}
...
TEST(test_different_submethods, integration_parallel_submethods) {
    ...
    double par_res = parallelIntegration(0, 2, 1000, Func);
...
TEST(test_diff_submethods, integr_seq_and_par_are_equival_xx) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    double par_res = parallelIntegration(0, 2, 100, Func);

    if (rank == 0) {
        double error = pow(10.0, -8);
        double seq_res = sequentialIntegration(0, 2, 100, Func);
        ASSERT_NEAR(seq_res, par_res, error);
    }
}

TEST(test_diff_submethods, integr_seq_and_par_are_equival_sinx) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
TEST(test_diff_submethods, integration_seq_and_par_are_equival_cosx) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
